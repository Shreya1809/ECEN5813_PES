...
cb_enum log_cb_init(CB_log_struct* src_ptr, size_t length)
{
    ...
    {
        src_ptr -> buffer = (int8_t *)malloc(length * sizeof(int8_t)); // allocates the length no of items for the buffer memory
        if(ptr -> buffer == NULL)
	    ...
	    log_cb_add(log_create(ERROR,ERROR,0,NULL),logger_queue);
        }
        else
        { //initialise size to given length, current item count in the buffer to 0, head and tail to null
            src_ptr->size = length;
            src_ptr->head = ptr->buffer;
            src_ptr->tail = ptr->buffer;
            src_ptr->count = 0;
...
cb_enum log_cb_remove(log_struct *logval_ptr , CB_log_struct* src_ptr)
{
	BEGIN_CRITICAL;
    cb_enum status = CB_SUCCESS;
    if (ptr == NULL || src_ptr->head == NULL || src_ptr->tail == NULL || src_ptr->buffer == NULL) //check for null pointer
    {
        status = CB_NULL_ERROR;
    }
